...
async function getSecrets<Type>({ connectors, preknowns, singleton }: getSecretsParams): Promise<Type | undefined> {

  const secretsArr: any = {};

  if (process.env.USE_AWS === "true") {
    if (connectors) handleSecretsFromAws(connectors, secretsArr);
    if (preknowns) handleSecretsFromAws(preknowns, secretsArr);
    if (singleton) secretsArr[singleton.name] = await getAwsSecret(process.env[`SINGELTON_SECRET_NAME`]!);
  }
  else {
    if (connectors) handleConnectorSecrets(connectors, secretsArr);
    if (preknowns) handlePreknownSecrets(preknowns, secretsArr);
    if (singleton) secretsArr[singleton.name] = await getSingletonEnvSecret(singleton);
  }
  return secretsArr;
}
...
const handleSecretsFromAws = async (secretsConfArr: connectorSecretConfig[] | preknownSecretConfig[], secretsArr: any) => {
  ...
  for (let i = 0; i < secretsConfArr.length; i++) {
    console.log('process.env[`${secretsConfArr[i].alias}_SECRET_NAME`: ', process.env[`${secretsConfArr[i].alias}_SECRET_NAME`]);
    console.log('secretsConfArr[i].alias: ', secretsConfArr[i].alias);
